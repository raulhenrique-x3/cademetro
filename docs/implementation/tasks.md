...
- **T1.1** Add deps: `@nestjs/jwt`, `bcryptjs`, `class-validator`, `class-transformer`,
  `@nestjs/throttler`.
  - Deps: — · Area: `package.json` · AC: `npm install` succeeds; types resolve.
- **T1.2** Global error filter → uniform error shape (`api/overview.md`).
  - Deps: — · Area: `src/common/` · AC: validation/404/429/500 map to envelope; no stack leak.
- **T1.3** Global `ValidationPipe` (whitelist + forbidNonWhitelisted + transform) + `ClassSerializerInterceptor`.
  - Deps: T1.1 · Area: `src/main.ts` · AC: unknown fields → 400; `passwordHash` excluded.
...
- **T2.1** Replace demo contract with domain model (`database/schema.md`); drop `Post`.
  - Deps: — · Area: `src/prisma/contract.prisma` · AC: models/enums/relations/indexes match schema.md.
...
- **T2.3** Seed script (minimal network: ≥1 line, 2 directions, stations in order).
  - Deps: T2.2 · Area: `prisma` seed · AC: seed idempotent; `db.orm.public.Line` returns data.

## Phase 3 — Authentication

- **T3.1** `AuthService` register/login: bcrypt hash/compare; unique email → 409.
  - Deps: T1.1, T2 · Area: `src/auth/` · AC: hash stored; wrong password → 401; dup email → 409.
- **T3.2** JWT issue (`@nestjs/jwt`, `JWT_SECRET`, exp 1h) + `JwtAuthGuard` (verify + status ACTIVE).
  - Deps: T3.1 · Area: `src/auth/` · AC: token verifies; bad/expired → 401; suspended → 403.
- **T3.3** `POST /auth/register`, `POST /auth/login`, `GET /auth/me`.
  - Deps: T3.2 · Area: `src/auth/` · AC: endpoints return `AuthResponse` / `UserDto`; docs reflect.
...
- **T8.2** TanStack Query provider + hooks (`useLines`, `useStations`, `useRecentReports`,
  ...
  - Deps: T8.1 · Area: `src/features/` · AC: reads/writes populate cache correctly.
- **T8.3** `use-realtime` hook: EventSource → update query cache; reconnect → snapshot refetch.
  - Deps: T8.1, Phase 7 · Area: `src/hooks/` · AC: SSE events update UI without manual refresh.
- **T8.4** Screens: Home, Line, Station, Report composer (modal), Login, Register, Profile,
  Moderation. Replace `explore.tsx` starter.
  - Deps: T8.2 · Area: `src/app/` · AC: flows in `screens.md` work.
- **T8.5** Map (maplibre) for station selection + on-demand location (`expo-location`).
  - Deps: T8.4 · Area: components · AC: station pickable; location sent only on submit.
- **T8.6** Components: status-badge, report-card, confidence-bar, type/station/direction pickers,
  report-composer, auth-form.
  - Deps: T8.4 · Area: `src/components/`, `src/features/` · AC: reusable; server data via hooks only.

## Phase 9 — Testing

- **T9.1** Unit: `ReliabilityService`, `StatusService`, `ReportsService`, `AuthService`.
  - Deps: Phase 5 · Area: `*.spec.ts` · AC: formulas + rules covered.
- **T9.2** Integration: ORM CRUD, report persistence, status aggregation, confirmations (test DB).
  - Deps: Phase 6 · Area: `*.spec.ts` · AC: DB-backed assertions pass.
- **T9.3** E2E: register → login → create report → recent → confirm → status.
  - Deps: Phase 6 · Area: `test/*.e2e-spec.ts` · AC: full flow green.
- **T9.4** Realtime tests: publish on create/confirm/dispute; status.updated; filtering.
  - Deps: Phase 7 · Area: `*.spec.ts` · AC: SSE events verified.
- **T9.5** Frontend: component, hook (useRealtime), and Zod schema tests (jest-expo).
  - Deps: Phase 8 · Area: frontend · AC: components/hooks/schemas tested.
